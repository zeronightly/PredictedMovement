...
namespace PredMovementCVars
{
	...
	static int32 DrawStaminaValues = 0;
	static FAutoConsoleVariableRef CVarDrawStaminaValues(
	...
	static FAutoConsoleVariableRef CVarClientAuthDisabled(
...
bool UPredictedCharacterMovement::HasListenServerMeshSmoothing() const
{
	...
	const IConsoleVariable* CVar = PredMovementCVars::GetNetEnableListenServerSmoothingCVar();
	return CVar && CVar->GetInt() != 0;
}

#if UE_5_06_OR_LATER
bool UPredictedCharacterMovement::ShouldExtrapolateListenServerMesh() const
{
	if (!bListenServerMeshExtrapolation || PredMovementCVars::ListenServerExtrapolation == 0)
	{
		return false;
	}
...
void UPredictedCharacterMovement::ResetMeshExtrapolation()
{
...
void UPredictedCharacterMovement::ServerAutonomousProxyTick(float DeltaSeconds)
{
...
void UPredictedCharacterMovement::SmoothClientPosition(float DeltaSeconds)
{
	FNetworkPredictionData_Client_Character* ClientData = GetPredictionData_Client_Character();
	...
	const float SmoothTime = PredMovementCVars::ListenServerExtrapolationSmoothTime >= 0.f
		? PredMovementCVars::ListenServerExtrapolationSmoothTime
...
void UPredictedCharacterMovement::OnTeleported()
{
	Super::OnTeleported();
...
#endif
{
...
#endif
	{
		...
		{
			// This runs after MoveAutonomous has already seeded mesh smoothing, so the mesh has to be held in place
			// and the offset rebased, or it pops by the full authority delta
			const bool bSmoothMesh = HasListenServerMeshSmoothing();
			const FVector PreAuthLocation = UpdatedComponent->GetComponentLocation();

			{
				const FScopedPreventAttachedComponentMove PreventMeshMove(bSmoothMesh ? CharacterOwner->GetMesh() : nullptr);
			...
			if (bSmoothMesh)
			{
				if (FNetworkPredictionData_Client_Character* ClientData = GetPredictionData_Client_Character())
				{
					const FVector AuthAdjust = PreAuthLocation - UpdatedComponent->GetComponentLocation();
					ClientData->MeshTranslationOffset = (ClientData->MeshTranslationOffset + AuthAdjust).GetClampedToMaxSize(ClientData->MaxSmoothNetUpdateDist);
					ClientData->OriginalMeshTranslationOffset = ClientData->MeshTranslationOffset;
					bNetworkSmoothingComplete = false;
				}
			}
